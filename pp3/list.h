...
template<class Element> class List {
 ...
 public:
    ...
    void CheckAll(int scopeLevel, ScopeTracker *tracker)                                     //pp3
        { for (int i = 0; i < NumElements(); i++)
             Nth(i)->Check(scopeLevel, tracker); }

    void CheckAll(int scopeLevel, ScopeTracker *tracker, char *parentName)                   // used for extended classes
        { for (int i = 0; i < NumElements(); i++)
             Nth(i)->Check(scopeLevel, tracker, parentName); }

    void SetUpAll(int scopeLevel, ScopeTracker *tracker)
        { for (int i = 0; i < NumElements(); i++)
             Nth(i)->SetUp(scopeLevel, tracker); }
