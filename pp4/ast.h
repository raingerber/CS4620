...
class Node 
{
  ...
  public:
    ...
    ClassDecl* FindParentClass();
    FnDecl* FindParentFn();
    bool isWithinLoop();

    bool inSamePosition(Node *compNode);

    bool messedUp;
    bool StoreExprError();
    void setErrorFlag() { messedUp = true; }
    bool errorFlagged() { return messedUp; }
};
...
class Identifier : public Node 
{
  ...
  public:
    ...
    void Check();
    Decl* Check(reasonT whyNeeded);
    Type* getType();

    bool isEqual(Identifier *testId) { // matt
        //printf("%s v. %s\n", name, testId->name);
        return strcmp(name, testId->name)==0;
    }
    bool isLengthId() {
