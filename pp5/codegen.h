...
#include "hashtable.h" // pp5
#include "ast_decl.h"
#include "ast_stmt.h"

              // These codes are used to identify the built-in functions
typedef enum { Alloc, ReadLine, ReadInteger, StringEqual,
               PrintInt, PrintString, PrintBool, Halt, NumBuiltIns, Null } BuiltIn;

class CodeGenerator {
  private:
    std::list<Instruction*> code;
    Hashtable<Decl*> *globaltable;
    Hashtable<FnDecl*> *globalFnTable; // could be combined with globaltable?
    Program *programPtr = NULL;
    char* currentBreakLabel;
    //bool mainHasBeenDefined;
    int TempNumber;
    Location *current_this_ptr = NULL;
  ...
  public:
    ...
    Location* getCurrentInstance();
    void setCurrentInstance(Location *loc);

    int IncrementTempNumber();
    int GetTempNumber();
    void ResetTempNumber();

/*    void setMainHasBeenDefined(bool b) { mainHasBeenDefined = b; }
    bool getMainHasBeenDefined() { return mainHasBeenDefined; }*/

    char* getCurrentBreakLabel();
    void  setCurrentBreakLabel(char *label);

    // easy access to the program stmt; screw traversing the tree
    // use it to find class declarations when needed
    void setProgramPtr(Program *program) { programPtr = program; }
    Program* getProgramPtr() { return programPtr; }

    ClassDecl* findClassDecl(char *name);
    FnDecl* findFnDecl(char *name);

    void storeGlobalFnDecl(FnDecl *fnDecl);
    FnDecl* findGlobalFnDecl(Identifier *identifier);

    Decl* findGlobal(char *name);
    ...
    Location *GenGlobalVar(char *name, VarDecl *varDecl);
    Location *GenLocalVar(char *name);

    Location *CustomGenLocalVar(char *name, int next_local_num);
