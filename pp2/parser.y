...
/***************************************************************/
%type <stmt>                Stmt ElseStmt StmtBlock
...
%type <expr>                Expr PossExpr Constant                   ParenExpr IntConst
%type <call>                Call
%type <lvalue>              LValue
%type <namedType>           ExtClause
%type <namedTypeList>       ImpClause IdList

%type <caseStmtList>        CaseStmtList CaseList
%type <defaultStmtList>     DefaultStmtList
/***************************************************************/
%%
Program   :    DeclList            { 
                                      @1; 
...
SwitchStmt   : T_SwitchStmt ParenExpr '{' CaseList DefaultStmtList '}'  { $$ = new SwitchStmt($2, $4, $5); } //'(' Expr ')'
;

CaseList     : CaseStmt CaseStmtList                     { ($$ = new List<CaseStmt*>)->AppendAll($1, $2); }
;
...
CaseStmt     : T_CaseStmt IntConst ':' StmtList          { $$ = new CaseStmt($2, $4); }
; 

DefaultStmtList  : DefaultStmt                               { ($$ = new List<DefaultStmt*>)->Append($1); }
                 | /* empty */                               { $$ = new List<DefaultStmt*>; };
;
...
Constant   : T_DoubleConstant                       { $$ = new DoubleConstant(@1, $1); }
...
IntConst   : T_IntConstant                          { $$ = new IntConstant(@1, $1); }
;
...
LValue    : Expr '.' T_Identifier    %prec '.'      { $$ = new FieldAccess($1, new Identifier(@3, $3)); }
          | Expr '[' Expr ']'        %prec '['      { $$ = new ArrayAccess(@1, $1, $3); }
          | T_Identifier                            { $$ = new FieldAccess(NULL, new Identifier(@1, $1)); }
;

Expr      : T_This                                  { $$ = new This(@1); }
          | LValue T_PostIncrement     %prec T_PostIncrement                 { $$ = new PostfixExpr($1, new Operator(@1, "++")); }                  // POSTFIX EXPRESSIONS
          | LValue T_PostDecrement     %prec T_PostDecrement                 { $$ = new PostfixExpr($1, new Operator(@1, "--")); }          
          | Call                                    { $$ = $1; }
          | LValue                                  { $$ = $1; }
          | Constant                                { $$ = $1; }
          //| IntConst                                { $$ = $1; }
	  ...
	  | Expr '=' Expr              %prec '='            { $$ = new AssignExpr($1, new Operator(@2, "="), $3); }
          | Expr '+' Expr              %prec '+'            { $$ = new ArithmeticExpr($1, new Operator(@2, "+"), $3); }
          | Expr '-' Expr              %prec '-'            { $$ = new ArithmeticExpr($1, new Operator(@2, "-"), $3); }
          | Expr '*' Expr              %prec '*'            { $$ = new ArithmeticExpr($1, new Operator(@2, "*"), $3); }
          | Expr '/' Expr              %prec '/'            { $$ = new ArithmeticExpr($1, new Operator(@2, "/"), $3); }
          | Expr '%' Expr              %prec '%'            { $$ = new ArithmeticExpr($1, new Operator(@2, "%"), $3); }
          
          | Expr '<' Expr              %prec '<'            { $$ = new RelationalExpr($1, new Operator(@2, "<"), $3); }
          | Expr T_LessEqual Expr      %prec T_LessEqual    { $$ = new RelationalExpr($1, new Operator(@2, "<="), $3); }
          | Expr '>' Expr              %prec '>'            { $$ = new RelationalExpr($1, new Operator(@2, ">"), $3); }
          | Expr T_GreaterEqual Expr   %prec T_GreaterEqual { $$ = new RelationalExpr($1, new Operator(@2, ">="), $3); }
          | Expr T_Equal Expr          %prec T_Equal        { $$ = new EqualityExpr($1, new Operator(@2, "=="), $3); }
          | Expr T_NotEqual Expr       %prec T_NotEqual     { $$ = new EqualityExpr($1, new Operator(@2, "!="), $3); }
          | Expr T_And Expr            %prec T_And          { $$ = new LogicalExpr($1, new Operator(@2, "&&"), $3); }
          | Expr T_Or Expr             %prec T_Or           { $$ = new LogicalExpr($1, new Operator(@2, "||"), $3); }
          | '-' Expr                   %prec UNARY          { $$ = new LogicalExpr(new Operator(@1, "-"), $2); }
          | '!' Expr                   %prec '!'            { $$ = new LogicalExpr(new Operator(@1, "!"), $2); }
;
